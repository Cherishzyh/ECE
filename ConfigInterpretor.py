...
class BaseImageOutModel():
    def __init__(self):
        ...
        self._cv_folder_list = []
        self._weights_path = []

    def IterateCase(self, root_folder_path, verbose=False, start_from='', only_folder=True):
    ...
    def LoadConfigAndModel(self, fold_path):
        ...
        if not (os.path.exists(config_path)):
            print('config_path not exists, please check!')
            return False
        self._config.LoadModelConfig(config_path)

        model_path = os.path.join(fold_path, 'model.py')
        if not (os.path.exists(model_path)):
            print('model_path not exists, please check!')
            return False
        sys.path.append(fold_path)
        from model import ResNeXt

        self._model = ResNeXt(3, 2)
        self._model.to(self.device)

        model_folder = os.path.join(fold_path, 'Model')
        self._cv_folder_list = [one for one in self.IterateCase(model_folder, only_folder=True)]

        for cv_folder in self._cv_folder_list:
            one_fold_weights_list = [one for one in self.IterateCase(cv_folder, only_folder=False) if one.is_file()]
            one_fold_weights_list = sorted(one_fold_weights_list,  key=lambda x: os.path.getctime(str(x)))
            self._weights_path.append(one_fold_weights_list[-1])
